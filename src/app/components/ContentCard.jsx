...
export default function ContentCard({ content }) {
  ...
  useEffect(() => {
    if (content?.videos?.length > 0) {
      ...
      setMediaUrl(getGoogleDriveDirectUrl(content.thumbnail, "thumbnail"));
    }
  }, [content]);
  ...
  const handleDownload = async (e) => {
    ...
    if (content?.videos?.length > 0) {
      url = isGoogleDriveVideo
        ? getGoogleDriveDirectUrl(content.videos[0], "download")
        : content.videos[0];
      fileName = `${content.title || "video"}.mp4`;
    } else {
      url = content?.thumbnail
        ? getGoogleDriveDirectUrl(content.thumbnail, "download")
        : mediaUrl;
      fileName = `${content.title || "image"}.jpg`;
    }
      ...
      {content?.videos?.length > 0 ? (
        <div className="relative w-full h-full z-10">
          ...
          <button
            ...
            className="absolute bottom-10 right-4 bg-black/40 hover:bg-black/60 p-2 rounded transition-colors"
            title={`Download ${
        ...
        <div className="relative w-full h-full">
          ...
          {isHovered && (
            <button
              onClick={handleDownload}
              className="absolute bottom-4 right-4 bg-black/40 hover:bg-black/60 p-2 rounded transition-colors z-10"
